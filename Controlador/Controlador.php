...
class Controlador
{
    ...
    public function agregarMedico($ide, $nom, $ape)
    {
        $medico = new Medico($ide, $nom, $ape);
        $gestorCita = new GestorCita();
        $registros = $gestorCita->agregarMedico($medico);
        // if ($registros > 0) {
        //     echo "<script>alert('Se insertó el medico con exito');
        //         window.location='index.php?accion=medicos'</script>";
        // } else {
    ...
    {
        $gestorCita = new GestorCita();
        $result = $gestorCita->consultarMedicoss();
    ...
    public function consultarMedicos()
    {
        $gestorCita = new GestorCita();
        $result = $gestorCita->consultarMedicoss();
        require_once 'Vista/html/consultarMedico.php';
    }
}
